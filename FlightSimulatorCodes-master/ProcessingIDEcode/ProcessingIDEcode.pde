...
void serialEvent(Serial port) //pembacaan serial
{
...
void MakeAnglesDependentOnMPU6050() //sudut pada MPU6050
{ 
  Bank =-Phi/5; 
  Pitch=Theta*10; 
  Azimuth=Psi;
}
void Horizon() //membuat indicator gyro
{ 
...
void ShowAzimuth() //text azimuth dan judul
{ 
  fill(50); 
  noStroke(); 
  rect(20, 470, 440, 50); 
...
void CompassPointer() //membuat jarum pada kompas
{ 
...
void Plane() //menggambar segitiga dan persegi panjang di tengah horizon
{ 
  fill(0); 
  strokeWeight(1); 
  stroke(0, 255, 0); 
...
void CircularScale() //untuk menggambar garis-garis lingkaran
{ 
...
void Axis() //untuk menggambar axis di dalam horizon
{ 
  stroke(255, 0, 0); 
  strokeWeight(3); 
  line(-115, 0, 115, 0); 
...
void ShowAngles() //membuat text pitch dan bank
{ 
  textSize(30); 
  fill(50); 
  noStroke(); 
...
void Borders() //untuk menggambar pinggiran horizon
{ 
  noFill(); 
  stroke(0); 
  strokeWeight(400); 
...
void PitchScale() //untuk menggambar garis-garis pada horizon
{  
  stroke(255); 
  fill(255); 
  strokeWeight(3); 
